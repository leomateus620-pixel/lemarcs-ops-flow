    ...
    <AppShell title="Mais">
      <GlassCard className="industrial-gradient mt-2 p-5">
        ...
        <h1 className="mt-1 font-display text-2xl font-black text-foreground">
          Equipe, perfil e demo
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">
          Acesso rápido a recursos mockados da v1 sem adicionar menus ao campo.
        </p>
      </GlassCard>
      ...
      <div className="mt-4 space-y-2">
        ...
        <MenuItem
          icon={Settings}
          label="Configurações mock"
          description="Preferências futuras do aplicativo"
        />
      ...
      <section className="mt-6">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <Mini
            label="Em campo"
            value={colaboradores.filter((c) => c.status === "Em campo").length}
          />
          <Mini
            label="Livres"
            value={colaboradores.filter((c) => c.status === "Disponível").length}
          />
        </div>
        <div className="mt-3 space-y-3">
          {colaboradores.map((c) => (
            <CollaboratorCard key={c.id} c={c} />
          ))}
        </div>
      </section>
    </AppShell>
    ...
    <GlassCard className="p-4">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-xs text-muted-foreground">
            {c.funcao} · {c.matricula}
          </div>
        </div>
        <span
          className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${statusStyles[c.status]}`}
        >
          {c.status}
        </span>
      </div>
      <div className="mt-3 flex items-center justify-between border-t border-border pt-3 text-xs">
        <span className="text-muted-foreground">
          OS hoje: <span className="font-bold text-foreground">{c.osHoje}</span>
        </span>
        <span className="text-muted-foreground">
          Horas mês: <span className="font-bold text-foreground">{c.horasMes}h</span>
        </span>
      </div>
    </GlassCard>
  );
